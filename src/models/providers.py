...
class OpenRouterProvider(LLMProvider):
    ...
    def __init__(self, config: ModelConfig, api_key: Optional[str] = None):
        ...
        if not api_key:
            # Provide helpful error message
            env_var_set = "OPENROUTER_API_KEY" in os.environ
            error_msg = (
                "OpenRouter API key not found.\n"
                "Please set the OPENROUTER_API_KEY environment variable in one of these ways:\n"
                "  1. Export in your shell: export OPENROUTER_API_KEY=your_key_here\n"
                "  2. Create a .env file in the project root with: OPENROUTER_API_KEY=your_key_here\n"
                "  3. Pass it as a parameter to create_provider()\n"
            )
            if env_var_set:
                error_msg += f"\nNote: OPENROUTER_API_KEY is in os.environ but is empty or None."
            raise ValueError(error_msg)
        
        # Use OpenAI client with OpenRouter's base URL
        self.client = openai.OpenAI(
            api_key=api_key,
            base_url="https://openrouter.ai/api/v1/"
        )
    
    @traceable(name=f"openrouter_generate", run_type="llm")
    def generate(self, prompt: str, **kwargs) -> tuple[str, float]:
...
def create_provider(config: ModelConfig, api_key: Optional[str] = None) -> LLMProvider:
    ...
    if config.provider == "openai":
        ...
        return OpenRouterProvider(config, api_key=api_key)
    else:
        raise ValueError(f"Unsupported provider: {config.provider}. Supported providers: 'openai', 'groq', 'openrouter'.")
